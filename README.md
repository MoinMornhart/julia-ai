...
- **Wöchentliche Selbstprüfung erkennt jetzt auch Grafikprobleme** – die tokenschonende Wochenprüfung (schaut ohne KI ins Start-Logbuch) achtet jetzt zusätzlich auf leere Fenster und degradierte Grafiktreiber, nicht nur auf Abstürze. So fällt ein Grafikproblem früher auf – gemeldet wird weiterhin nur mit deiner Zustimmung und bereinigt (nie IP/Tokens).
- **Leeres Fenster heilt sich jetzt auch bei Grafik-Problemen selbst** – auf manchen PCs (z. B. mit zickigem Grafiktreiber) blieb die Oberfläche leer, ohne dass ein Absturz gemeldet wurde – die Selbstheilung sprang deshalb nicht an. Jetzt erkennt Julia genau diesen Fall: Bleibt das Fenster leer, stellt sie automatisch auf Software-Grafik um und startet einmal neu – dann ist das Bild meist wieder da. Außerdem schreibt sie beim Start deutlich mehr ins Logbuch, um solche Fälle schneller einzugrenzen.
