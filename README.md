...
- **Treiber-Hinweis bei Grafikproblemen** – meldet deine Grafikkarte keine Treiber-Infos (typisch bei einem leeren Fenster), zeigt Julia in der Reparatur-Sektion einen Hinweis mit dem **Link zur offiziellen Treiberseite** deines Herstellers (NVIDIA/AMD/Intel). Julia installiert **nichts** selbst – du entscheidest, ob du den Treiber holst.
- **Ein-Klick-Grafik-Reparatur** – bleibt das Fenster mal leer oder flackert es, kannst du in den Einstellungen (System) mit einem Klick auf „Auf Software-Grafik umstellen & neu starten" gehen. Das ist der Rettungsanker gegen Grafiktreiber-Zicken (dasselbe, was Technik-Profis mit `--disable-gpu` machen) – umkehrbar, ohne Eingriff ins System. Danach genügt „Wieder normale Grafik versuchen".
