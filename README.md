...
- **MCP-Server per Drag-and-Drop hinzufügen** – zieh einfach eine `mcp.json` in die MCP-Einstellungen (oder klick zum Auswählen): Julia erkennt die enthaltenen Server (der verbreitete `mcpServers`-Stil, lokale Programme wie auch Web-Adressen) und übernimmt sie. Jeder Aufruf läuft weiter über die Ampel, und die Server sind zunächst „nicht vertraut".
