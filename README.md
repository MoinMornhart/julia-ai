...
- **Absturz beim Start behoben – die Oberfläche lädt wieder** – auf manchen Rechnern baute sich die Oberfläche gar nicht mehr auf, weil sich zwei Skripte im Chatfenster denselben Namen teilten (doppelt vergebenes `mitZeitlimit`/`md`) und dadurch das Haupt-Skript komplett abbrach – in der Folge fehlten weiteren Bausteinen ihre Helfer. Behoben; zusätzlich stellt jetzt ein automatischer Test sicher, dass sich Oberflächen-Skripte nicht mehr gegenseitig ausknocken.
- **VibeWorks: Anmelden per Konto ohne Schlüssel-Kopieren (Geräte-Anmeldung)** – neben dem API-Schlüssel gibt es jetzt in den Einstellungen „Mit Konto anmelden (Gerät)": Julia holt sich selbst einen Code, die VibeWorks-Seite öffnet sich, du erlaubst den Zugriff **einmal** – fertig. Kein Schlüssel mehr zum Kopieren. Der Zugang wird sicher abgeholt und liegt nur verschlüsselt auf dem PC (die KI sieht ihn nie); die einmalige Freigabe durch dich bleibt die Sicherheitsgrenze.
- **VibeWorks-Anmeldung friert nicht mehr ein** – reagiert der VibeWorks-Server einmal nicht, bricht Julia die Schlüssel-Prüfung nach kurzer Zeit sauber ab und zeigt einen Netz-Hinweis, statt dass die Anmelde-Box ewig lädt („nichts lädt"). Eine kleine Schutzfunktion, damit ein hängender Server die Oberfläche nicht blockiert.
- **Agenten mit Prüfer-Schleife (BETA)** – Julia kann eine Aufgabe jetzt im Zusammenspiel zweier Rollen lösen: eine „Ersteller"-Rolle macht einen Entwurf, eine „Prüfer"-Rolle kritisiert ihn gegen das Ziel, und es wird nachgebessert – bis der Prüfer zufrieden ist oder die (kleine) Rundenzahl erreicht ist. So werden Ergebnisse gründlicher. Beide Rollen denken nur (kein PC-Zugriff), und es gibt das nur bei eingeschalteten BETA-Agenten. Dritter und letzter Baustein der Agenten-Struktur.
- **Agenten-Rollen delegieren (BETA)** – Julia kann jetzt eine fokussierte Teilaufgabe an eine deiner Rollen abgeben (z. B. eine „Kritiker"- oder „Rechercheur"-Rolle um eine Zweitmeinung bitten) und deren Antwort nutzen. Der Rollen-Agent **denkt nur nach – kein PC-Zugriff, keine Werkzeuge** – und das Ganze gibt es nur, wenn du BETA-Agenten eingeschaltet hast. Zweiter Baustein der Agenten-Struktur.
- **Agenten-Rollen (BETA)** – im BETA-Bereich kannst du jetzt eigene, spezialisierte Julias anlegen: eine Rolle bekommt einen Namen und eine Zusatz-Anweisung (z. B. „Coder" oder „Rechercheur"), und die aktive Rolle prägt Julias Verhalten zusätzlich. Ist BETA aus, ist alles komplett versteckt; Sicherheit und die Ampel bleiben unberührt, und die KI kann Rollen nicht selbst anlegen oder aktivieren. Erster Baustein für eine größere Agenten-Struktur (Planer/Prüfer folgen).
- **Sicherheit erneut nachgeschärft** – nach einem automatischen Repo-Check zwei kleine Härtungen: unsichtbare Steuerzeichen stehen im Quelltext nur noch als lesbare Escapes, und das Ändern von Einstellungen ist zusätzlich gegen manipulierte Sonderschlüssel abgesichert. Verhalten bleibt gleich.
- **Mehrstufige Grafik-Rettung bei zickigen Treibern** – bleibt das Fenster leer, probiert Julia jetzt automatisch nacheinander verschiedene Grafik-Verfahren durch (moderne Treiber, ein älteres/verträglicheres, OpenGL, dann ein reiner Software-Renderer, zuletzt ganz ohne GPU) und merkt sich das, mit dem es klappt. So findet sie auf mehr PCs von selbst einen Weg, ohne dass du etwas einstellen musst.
