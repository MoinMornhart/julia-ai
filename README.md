...
- **Nicht mehr ständig Benachrichtigungen beim Minecraft-Bauen** – Julia hat bisher bei **jedem** Minecraft-Ereignis eine Windows-Benachrichtigung geschickt (auch beim Bauen/Essen). Jetzt meldet sie standardmäßig nur noch **Wichtiges** (z. B. getrennt, gestorben, Gefahr, Ziel erreicht). Im Minecraft-Bereich lässt sich das umstellen: „Nur Wichtiges" (Standard), „Alle" oder „Keine". Der Minecraft-Bereich zeigt trotzdem alles live an.
- **Doppelte MCP-Server bereinigt** – stand ein Server (z. B. VibeWorks) doppelt in den MCP-Einstellungen, wird das jetzt automatisch entdoppelt (nach Adresse/Befehl) – beim Hinzufügen/Import und einmalig beim Start. Der Anzeigefehler, bei dem im Overlay wörtlich `{name}` statt des Namens stand, ist ebenfalls behoben.
- **Leeres Fenster & Fehl-Alarm „Oberfläche leer" wirklich behoben** – die eigentliche Ursache ist gefunden: die mitgelieferten Texte kamen im gepackten Programm gar nicht dort an, wo die Oberfläche sie zum Beschriften braucht (ein technischer Sandbox-Grund). Dadurch blieben die Beschriftungen leer, und Julia hielt das Fenster fälschlich für „kaputt" und startete in einer Schleife neu – obwohl es eigentlich funktionierte. Jetzt holt sich Julia die Texte auf einem zuverlässigen Weg und füllt die Oberfläche notfalls selbst; und ein „leere Beschriftungen"-Verdacht löst **nie** mehr einen Neustart oder die Fehlermeldung „Oberfläche bleibt leer" aus. Das war der hartnäckige Fehler, der auf manchen PCs die App unbenutzbar machte.
- **Läuft jetzt auf (fast) jedem Gerät – Absturzschutz (XXL-Update)** – zwei große Robustheits-Verbesserungen, damit Julia auch auf schwächeren/zickigen PCs sicher startet und nicht mehr wortlos verschwindet: (1) Ein **globaler Fangschirm** fängt jetzt unerwartete Fehler ab – **beim Start** gibt es eine klare Meldung statt eines stummen Absturzes, und **im Betrieb** läuft Julia bei einem Hintergrundfehler einfach weiter, statt ganz abzustürzen. (2) Ist Julias **Datenordner gesperrt** (Schreibrechte, Virenscanner o. ä.) – eine häufige Ursache, dass die App gar nicht erst aufgeht –, weicht sie automatisch auf einen Ersatzordner aus, statt am Start zu scheitern. Alles landet weiterhin nur lokal im Start-Logbuch (nichts wird verschickt).
- **Gedanken-Box erscheint sofort – und der Denkaufwand ist jetzt einstellbar** – die „Gedanken"-Box taucht jetzt **sofort** beim Antwortstart auf (aufgeklappt, damit man direkt sieht, dass Julia arbeitet) und füllt sich live; kommt gar kein Denk-Schritt, verschwindet sie wieder. Neu lässt sich der **Denkaufwand** (z. B. auf „mittel") auch bei OpenAI-kompatiblen Anbietern einstellen – er wird als `reasoning_effort` mitgeschickt. Kennt ein Modell den Regler nicht, lässt Julia ihn für dieses Modell automatisch wieder weg (kein Fehler). Wie viel „Nachdenken" sichtbar wird, hängt weiterhin vom Modell/Anbieter ab.
- **Ein einziges Merk-System (weniger Werkzeuge)** – „Gedächtnis" und „Lern-Notizen" waren zwei getrennte Werkzeug-Gruppen (6 Werkzeuge). Julia nutzt jetzt **eine** Gruppe (merken/lesen/löschen): standardmäßig das globale Gedächtnis, mit dem Schalter „projekt" die projektbezogenen Notizen im Arbeitsordner (die mit dem Projekt wandern). Weniger Werkzeuge = schneller und tokenschonender – und nichts geht verloren, beide Speicher bleiben.
- **Auto-Update läuft wieder zuverlässig** – bei manchen brach das automatische Update mittendrin ab („Prozesse noch aktiv"), weil sich der Installer beim harten Beenden versehentlich selbst mit abgeschossen hat. Behoben: Julia beendet jetzt nur noch ihre eigenen Programme (nicht den laufenden Installer). Hinweis: Diese Verbesserung greift ab dem nächsten Installer – wenn dein Update feststeckt, einmal die aktuelle Version von der Releases-Seite von Hand installieren, danach läuft's automatisch.
- **Denk-Schritt jetzt auch bei mehr Anbietern sichtbar** – die eingeklappte „Nachdenken"-Box erscheint jetzt auch bei OpenAI-kompatiblen Anbietern, die ihren Reasoning-Schritt mitsenden (z. B. DeepSeek), nicht nur bei Anthropic.
- **Videos schneiden & Thumbnails** – Julia kann jetzt auf Zuruf Videos **schneiden/trimmen** (von–bis), **mehrere zusammenfügen** und **Thumbnails** (Standbilder) erstellen – z. B. aus deinen Gaming-Clips. Läuft komplett lokal über ffmpeg, nichts wird hochgeladen; jede neue Datei wird einmal freigegeben (GELB). Die Werkzeuge sitzen in der abschaltbaren Kategorie „Videos". (Braucht ffmpeg auf dem PC – fehlt es, sagt Julia klar Bescheid.)
