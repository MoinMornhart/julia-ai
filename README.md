...
- **Sehr lange Gespräche bleiben schlank** – bei sehr langen Chats behält Julia jetzt automatisch nur die jüngsten Runden im aktiven Kontext (die neueren bleiben, uralte fallen weg), damit die Kosten nicht immer weiter steigen. Das passiert an einer sicheren Stelle, sodass nichts durcheinandergerät.
- **Weniger Tokens bei langen Gesprächen (eigene Anbieter)** – bei OpenAI-kompatiblen Modellen schickt Julia jetzt nur noch die letzten Screenshots mit, statt bei jeder Runde alle alten Bilder erneut. Das spart spürbar Tokens/Kosten in langen Sitzungen, ohne dass der Gesprächsfaden verloren geht (bei Anthropic-Modellen macht das die Server-Seite schon).
- **KI kann beim Einrichten nach einem Schlüssel fragen – ohne ihn zu sehen** – braucht Julia für ein Setup (z. B. einen MCP-Server/Dienst) einen Zugang, kann sie jetzt eine **Eingabe-Box** öffnen: Du tippst den Wert dort ein, er wird **verschlüsselt** auf dem PC gespeichert – die KI bekommt nur „hinterlegt" zurück und **sieht den Wert nie**. Abbrechen ist jederzeit möglich.
- **KI darf Aussehen/Vorlieben anpassen – Wichtiges bleibt gesperrt** – Julia kann jetzt auf Wunsch unkritische Einstellungen ändern (z. B. Design hell/dunkel, Akzentfarbe, Brainstorming, Lernen) – bei allem außer der Blase kommt vorher eine Bestätigungs-Box. **Gesperrt** bleiben wichtige/sicherheitsrelevante Dinge: Anbieter und Anbieter-Adresse (kann die KI jetzt nicht mehr umbiegen), API-Schlüssel, Sandbox, BETA, Diagnose-Versand, MCP, „Allem zustimmen", Kosten, Mikrofon – die ändert nur der Nutzer selbst.
- **Token-Schutz bei Geheimnissen** – Julia erkennt jetzt echte Zugangs-Schlüssel/Tokens (GitHub, OpenAI, Google, AWS, Stripe, VibeWorks u. v. m. sowie generell sehr lange, zufällige Schlüssel). Trägst du versehentlich einen echten Token als **Namen** eines Geheimnisses ein, weist Julia das ab – so kann kein Schlüssel dort landen, wo die KI nur den Namen sieht. Der Wert selbst bleibt wie bisher nur verschlüsselt auf dem PC.
