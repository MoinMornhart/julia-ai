...
- **Empty window solved for good – emergency label fill** – on some PCs the window stubbornly stayed empty (just the frame, no labels), which led to a restart loop. The cause was **not** a graphics problem, but that the labels were filled only through a fragile detour inside the window – if that arrived empty, everything stayed empty. Julia now fills the interface, if needed, **directly from the bundled texts** (independent of that detour), right at load and again before it even assumes an "empty window". So the window is labelled even if something else is stuck on the PC – no more restart loop.
...
- **Cut videos & thumbnails** – Julia can now **cut/trim** videos on request (from–to), **join several** together and create **thumbnails** (still frames) – e.g. from your gaming clips. Runs entirely locally via ffmpeg, nothing is uploaded; each new file is approved once (yellow). The tools live in the switchable "Videos" category. (Needs ffmpeg on the PC – if it's missing, Julia says so clearly.)
