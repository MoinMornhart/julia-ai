...
// Nur Heimnetz: private IPv4-Bereiche, Link-Local, Loopback und private IPv6.
...
function lanAdressen() {
  const liste = [];
  for (const [karte, eintraege] of Object.entries(os.networkInterfaces())) {
    for (const e of eintraege || []) {
      if (e.family !== 'IPv4' || e.internal || !privateAdresse(e.address) || e.address.startsWith('169.254.')) continue;
...
class HandyServer extends EventEmitter {
  ...
  constructor({ tresor, texte, beiNachricht, protokoll = () => {}, adressen = lanAdressen }) {
    super();
    this.tresor = tresor;
    this.texte = texte;
  ...
  // Zertifikat aus dem Tresor; neu, wenn es fehlt oder bald abläuft.
  _zertifikat() {
    const e = this.tresor.lesen('handy') || {};
    if (e.zertifikat && e.privat_schluessel) {
      try {
    ...
    this.tresor.schreiben('handy', { zertifikat: neu.zertifikat, privat_schluessel: neu.schluessel });
  ...
  koppelnStarten() {
    ...
    const adresse = this.adressenFinden()[0];
...
module.exports = { HandyServer, privateAdresse, hostErlaubt, lanAdressen, geraetName, qrMatrix };
